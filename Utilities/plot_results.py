...
def plot_values_global_modular(values,save_to, verbose=0):
    """
...
def plot_values_nodal_connections(values,save_to, verbose=0):
    """
    This functions plots the LDA results for the Global and Local frameworks.
	...
	if type(values) == list:
		scores = values[:]
		labels = ('Nodal', 'Connections')
		x_cords1 = np.arange(len(labels))
		plt.ylim([0.5,1.01])
		plt.bar([0], float(scores[0]), align='center', width=0.50, color=['#E63434'],label='Nodal',alpha=0.65)
		plt.bar([1], float(scores[1]),align='center', width=0.50, color=['#09BA15'],label='Connections',alpha=0.65)
		#plt.bar([2], float(scores[2]),align='center', width=0.70, color=['#4D4DFF'],label='Leading_Eigenvector',alpha=0.65)
		plt.tick_params(axis='x',labelsize=12)
		plt.xticks(x_cords1, labels)
		plt.ylabel('Accuracy_LOOCV')
		plt.title('SVM classification using all the 3 Groups',fontsize=13)
		plt.legend(loc=4,prop={'size':10})
		plt.savefig(save_to + 'results_SVM.png', dpi=300)
		if verbose:
			plt.show()
